...
class WeightedGraph(Graph):
    ...
    def floyd_warshall(self):
        ...
        # set all to infinity
        for vertex1 in all_ids:
            dist[vertex1] = dict()
            for vertex2 in all_ids:
                dist[vertex1][vertex2] = WeightedGraph.INFINITY
            dist[vertex1][vertex2] = 0

        #add all edge weight to the dictionary
        all_vertex_obj = self.get_vertices()

        for vertex in all_vertex_obj:
            neighbors_weights = vertex.get_neighbors_with_weights()

            for neighbor, weight in neighbors_weights:
